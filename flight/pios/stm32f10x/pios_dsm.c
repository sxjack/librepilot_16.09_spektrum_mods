...
static int PIOS_DSM_UnrollChannels(struct pios_dsm_dev *dsm_dev)
{
    ...
    /* Code to autodetect the resolution based on received_data[1] could
       go here, but it would only work for 'A' (aka internal) remotes and 
       would, in my opinion, be dangerous as there is no way of telling 
       if the user is not using an 'A' remote.
    */
