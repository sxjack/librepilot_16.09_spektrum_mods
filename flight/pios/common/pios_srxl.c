...
static void PIOS_SRXL_UnrollChannels(struct pios_srxl_state *state)
{
    ...
    case SRXL_SPEKTRUM_A5_HEADER:
      for (i = 2; i < 16; i += 2) {
	channel = 0x0f & ((uint16_t) received_data[i] >> 3);
	channel_value = 0x07ff & (((uint16_t) received_data[i] << 8) | ((uint16_t) received_data[i + 1])); 
	state->channel_data[channel] = channel_value;
      }
      break;
    }
...
static bool PIOS_SRXL_Validate_Checksum(struct pios_srxl_state *state)
{
    ...
    uint8_t i    = 0;
    uint16_t crc = 0, checksumMpx, checksumSpek;
    ...
    checksumMpx  = (((uint16_t)(state->received_data[i] << 8) |
		     (uint16_t)state->received_data[i + 1]));

    checksumSpek = (((uint16_t)(state->received_data[i + 1] << 8) |
		     (uint16_t)state->received_data[i]));

    if (crc == checksumMpx) {

      // Multiplex or Spektrum AR7700
      
      return 1;

    } else if (crc == checksumSpek) {
...
static void PIOS_SRXL_UpdateState(struct pios_srxl_state *state, uint8_t b)
{
    ...
    if (state->byte_count < (SRXL_HEADER_LENGTH + state->data_bytes + SRXL_CHECKSUM_LENGTH)) {
	    ...
	    case SRXL_V2_HEADER:
	      ...
	      break;
	    case SRXL_SPEKTRUM_A5_HEADER:
	      state->data_bytes = SRXL_A5_CHANNEL_DATA_BYTES;
	      break;
	    default:
